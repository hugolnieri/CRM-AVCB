...
| **`0013_cnae.sql`** | **NÃO** | CNAE em leads/clientes e prefixos em `tipos_servico` |
...
## Por que a 0013 está pendente

O conector do Supabase caiu no meio da sessão e não voltou. Ela **não** foi
aplicada pelo SQL Editor de propósito: aplicar SQL à mão não registra nada em
`supabase_migrations.schema_migrations`, então o histórico local e o remoto
passariam a divergir em silêncio — e a próxima pessoa a rodar as migrations
não teria como saber o que já existe.

Enquanto ela não roda, o cadastro de lead e de cliente falha ao salvar (tenta
gravar coluna que não existe). O resto do sistema segue funcionando.
